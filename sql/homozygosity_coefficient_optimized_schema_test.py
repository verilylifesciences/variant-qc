...
"""Tests for query homozygosity_coefficient_optimized_schema.sql.
...
class QueryTest(bq_test_case.BQTestCase):
  ...
  def setUpClass(cls):
    ...
    cls.sql_to_test = open(
        os.path.join(
            ...
            "homozygosity_coefficient_optimized_schema.sql"),
        "r").read()
  ...
  def create_mock_tables(cls):
    ...
    cls.client.create_table_from_query(
        TestConstants.OPT_MULTISAMPLE_VARIANTS_TABLE_INPUT,
        cls.basic_table_name)

    # Table created specifically for this test.
    cls.src_table_name = cls.client.path("opt_multisample_variant_table")
    cls.client.create_table_from_query("""
SELECT * FROM UNNEST([
STRUCT<reference_name STRING,
    ...
    ('chr1', 123, 124, 'A', 8, ARRAY[STRUCT('T', 1, 0.125)], ARRAY[STRUCT('sample1', ARRAY[0, -1]),
                                                                   STRUCT('sample3', ARRAY[0, 1]),
                                                                   STRUCT('sample5', ARRAY[-1, -1])],
                                                             ARRAY['sample2', 'sample4', 'sample6']),
    ('chr1', 456, 457, 'G', 8, ARRAY[STRUCT('C', 3, 0.375)], ARRAY[STRUCT('sample2', ARRAY[-1, -1]),
                                                                   STRUCT('sample4', ARRAY[1, -1]),
                                                                   STRUCT('sample5', ARRAY[1, 0]),
                                                                   STRUCT('sample6', ARRAY[1, 1])],
                                                             ARRAY['sample1', 'sample3']),
    ('chr1', 789, 790, 'C', 10, ARRAY[STRUCT('G', 6, 0.6)], ARRAY[STRUCT('sample2', ARRAY[-1, -1]),
                                                                  STRUCT('sample4', ARRAY[1, 1]),
                                                                  STRUCT('sample5', ARRAY[1, 1]),
                                                                  STRUCT('sample6', ARRAY[1, 1])],
                                                            ARRAY['sample1', 'sample3'])
