...
"""Tests for query sex_chromosome_heterozygous_haplotypes.sql.
  ...
  def setUpClass(cls):
    ...
    cls.sql_to_test = open(
        os.path.join(
            ...
            "sex_chromosome_heterozygous_haplotypes.sql"),
        "r").read()
...
STRUCT<reference_name STRING,
    ...
    ('chrY', 456, 457, 'C', ARRAY[STRUCT('A')], ARRAY[STRUCT('sample1', ARRAY[-1, -1]),
                                                      STRUCT('sample2', ARRAY[-1, 1]),
    ...
    ('X',  789, 790, 'C',  ARRAY[STRUCT('G'),
                                 STRUCT('T')],  ARRAY[STRUCT('sample1', ARRAY[1, 2]),
                                                      STRUCT('sample4', ARRAY[1]),
                                                      STRUCT('sample5', ARRAY[2])]),
    ('X',  789, 790, 'C',  ARRAY[STRUCT('CC')], ARRAY[STRUCT('sample2', ARRAY[1, 1])]),
    ('Y',  789, 791, 'CA', ARRAY[STRUCT('C')],  ARRAY[STRUCT('sample3', ARRAY[0, 1])])
  ])""", cls.src_table_name)

  def test(self):
    sql = Template(self.sql_to_test).render(
        {"GENOME_CALL_OR_MULTISAMPLE_VARIANT_TABLE": self.src_table_name,
         "MALE_SAMPLES_QUERY": "'sample1', 'sample2', 'sample3', 'sample4', 'sample5'",
         "HIGH_QUALITY_CALLS_FILTER": "TRUE"})
    expected = [
        #  reference_name, start_position, end_position, reference_bases,
        #         alt_concat, name, genotype
        ("chrX", 123, 124, "A",  "T",   "sample3", "1,0"),
        ("chrX", 123, 124, "A",  "T",   "sample4", "0,1"),
        ("chrY", 456, 457, "C",  "A",   "sample3", "0,1"),
        ("X",    789, 790, "C",  "G,T", "sample1", "1,2"),
        ("Y",    789, 791, "CA", "C",   "sample3", "0,1")
        ]
  ...
  def test_basic_input(self, table):
    ...
    # not contain any variants on chromosomes X or Y.
    expected = []
    self.expect_query_result(
        query=sql, expected=expected, enforce_ordering=False)
