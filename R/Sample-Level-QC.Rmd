...
sample_info <- perform_bqquery(sql = params$SAMPLE_INFORMATION_QUERY,
                              params = params)
ANCESTRY_PLOT_HEIGHT <- 2 * length(unique(sample_info$ancestry))
...
ggplot(joined_result, aes(y = number_of_calls, x = ancestry, fill = ancestry)) +
  geom_boxplot() +
  ...
  xlab("Ancestry") +
  ggtitle("Box plot: Count of variant calls per genome by ancestry") +
  theme(axis.text.x = element_text(angle = 50, hjust = 1))
```

```{r genomeCalls, fig.align = "center", fig.width = 10, comment = NA}
p <- ggplot(joined_result) +
  geom_point(aes(x = name, y = number_of_calls, color = sex)) +
...
```{r homozygositySummary, fig.align = "center", fig.width = 12, comment = NA}
  ggplot(joined_result %>% filter(!stringr::str_detect(reference_name, "^GL")),
         aes(y = F, x = reference_name, color = sex)) +
  geom_boxplot() +
  facet_grid(sex ~ .) +
  scale_y_continuous(labels = comma) +
...
```{r comment = NA}
if (1 >= length(params$MULTISAMPLE_VARIANT_TABLE)) {
  result <- data.frame()
  cat("not run")
} else {
...
ggplot(joined_result %>%
         filter(!stringr::str_detect(reference_name, "^(GL|M|m)")),
       aes(y = titv, x = reference_name, color = sex)) +
  geom_boxplot() +
