...
# Copyright 2019 Google Inc., Verily Life Sciences LLC. All rights reserved.
...
params:
  ...
  # Zero-based b38 coordinates for SHANK1 per https://www.ncbi.nlm.nih.gov/gene/50944
  SHANK1_START: 50659256
  SHANK1_END: 50719783
...
Let's take a look at a few of the variants within SHANK1 via BigQuery.
```{r comment = NA}
result <- perform_bqquery(sql_path = "../sql/variant_level_data_for_shank1.sql",
                          params = params)
```
...
Let's take a look at a few non-variant segments within SHANK1 (if any, it will depend on the input VCF files):
```{r comment = NA}
result <- perform_bqquery(sql_path = "../sql/non_variant_segments_shank1.sql",
                          params = params)
```
Number of rows returned by this query: **`r if(is.null(result)) { "None" } else { nrow(result) }`**.
